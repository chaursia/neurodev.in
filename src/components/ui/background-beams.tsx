...
export const BackgroundBeams = React.memo(
  ({ className }: { className?: string }) => {
    ...
    const [animationValues, setAnimationValues] = useState<{y2: number; duration: number; delay: number}[]>([]);

    useEffect(() => {
      // Initialize with a timeout to keep render pure
      const timeout = setTimeout(() => {
        setAnimationValues(paths.map(() => ({
          y2: 93 + Math.random() * 8,
    ...
    if (animationValues.length === 0) return null;
